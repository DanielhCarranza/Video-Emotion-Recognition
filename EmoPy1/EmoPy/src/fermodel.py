...
class FERModel:
    ...
    **Example**::
    ...
    def predict(self, image_file):
        ...
        normalized_prediction = [x/sum(prediction[0]) for x in prediction[0]]
        for emotion in self.emotion_map.keys():
            print('%s: %.1f%%' %
                  (emotion, normalized_prediction[self.emotion_map[emotion]]*100))
        dominant_emotion_index = np.argmax(prediction[0])
        for emotion in self.emotion_map.keys():
            if dominant_emotion_index == self.emotion_map[emotion]:
                dominant_emotion = emotion
                break
        return dominant_emotion, normalized_prediction[self.emotion_map[dominant_emotion]]
